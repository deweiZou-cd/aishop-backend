...
func GetRouters(r *gin.Engine) {
	...
	user := r.Group("/user", middleware.LoginNeed)
	{
		...
		userInfo := user.Group("/userInfo")
		{
			...
			//修改用户个人简介	表单传入参数：1.自我介绍内容 返回值:自我介绍内容
		...
		goods := user.Group("/goods")
		{
			...
			//获取某一用户的商品信息 需要展示的数据：1.商品名 2.商品所属人员 3.商品图片（一张） 4.价格
	...
	// 添加聊天模块
	chat := r.Group("/chat", middleware.LoginNeed)
	{
		chat.POST("/send", controller.SendMessage)
		chat.GET("/history", controller.GetChatHistory)
		chat.GET("/ws", controller.HandleWebSocket)
	}
	// 支付页面路由
}
